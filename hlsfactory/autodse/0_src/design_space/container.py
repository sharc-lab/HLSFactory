        ...
        for filename, content in self.template_files.items():
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if match:
                    ...
                    # Store pragma location
                    # Find region (function or loop)
                    # Extract region from placeholder if possible, otherwise find it
                    region_match = re.search(r'__\w+__(\w+)', placeholder)
                    if region_match:
    ...
    def _find_region(self, lines: List[str], pragma_line_index: int) -> str:
        ...
        for i in range(pragma_line_index - 1, -1, -1):
            ...
            if re.search(loop_pattern, line):
                # Check for a label on the same line
                label_match = re.match(label_pattern, line)
                if label_match:
                    return label_match.group(1)
                
                # Check previous non-empty line for a label
                for j in range(i - 1, -1, -1):
                    prev_line = lines[j].strip()
                    if prev_line:
                        label_match = re.match(label_pattern, prev_line)
                        if label_match:
                            return label_match.group(1)
