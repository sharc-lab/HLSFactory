...
class CSynthAnalyzer:
                ...
                if overall_lat is not None:
                    ...
                    if ii is not None and ii.text:
                        ...
                        if ii_text.lower() == 'undef' or ii_text == '-':
                            ...
                            try:
                                metrics['PipelineInitiationInterval'] = max(map(int, ii_text.split('~')))
    ...
    def parse_transform_messages(self):
        ...
        log_file = os.path.join(self.csynth_report_dir, '.autopilot', 'db', 'autopilot.flow.log')
        ...
        with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                if 'HLS 214-186' in line or 'HLS 214-188' in line:
                    ...
                    m = ur_complete.search(line) if is_complete else ur_partial.search(line)
                    if m:
    ...
    def _parse_message_xml(self):
        ...
        if not os.path.isfile(msg_file):
            raise FileNotFoundError(f"Neither autopilot.flow.log nor message_syn.xml found in '{self.csynth_report_dir}'")
