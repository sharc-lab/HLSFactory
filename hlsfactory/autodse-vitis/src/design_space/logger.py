    ...
    def _initialize_levelheap_trace(self, work_dir: str):
        ...
        try:
            if trace_exists:
                ...
                self.print(f"[TeeLogger] LevelHeap trace resumed (existing file): {self.levelheap_trace_file}")
                if final_trace_exists:
                    self.print(f"[TeeLogger] Final trace JSON resumed (existing file): {final_trace_json}")
            else:
        ...
        try:
            ...
            if self.final_trace_json_file.exists():
                ...
                try:
                    ...
                    self.print(f"[TeeLogger] Final trace JSON resumed with {len(existing_data)} existing entries: {self.final_trace_json_file}")
                except json.JSONDecodeError as e:
                    self.print(f"[TeeLogger] Warning: Existing final_trace.json is corrupted, recreating: {e}")
                    # Backup the corrupted file
                    backup_file = self.final_trace_json_file.with_suffix('.json.backup')
                    self.final_trace_json_file.rename(backup_file)
                    self.print(f"[TeeLogger] Backed up corrupted file to: {backup_file}")
                    # Create new file
                    with open(self.final_trace_json_file, 'w') as f:
                        json.dump({}, f, indent=2)
                    self.final_trace_json_initialized = True
                    self.print(f"[TeeLogger] Final trace JSON recreated: {self.final_trace_json_file}")
            else:
