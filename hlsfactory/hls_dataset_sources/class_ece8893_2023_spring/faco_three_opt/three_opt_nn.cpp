...
struct Segment {
    ...
    /* Returns length of a segment (non-recursive; HLS rejects size()->size()). */
    [[nodiscard]] uint32_t size() const {
        const uint32_t first = is_reversed_ ? last_ : first_;
        const uint32_t last = is_reversed_ ? first_ : last_;
        if (first <= last) {
            return last - first + 1;
        }
        return len_ - first + last + 1;
    }
