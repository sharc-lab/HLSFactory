    ...
    for(int ti = 0; ti <= (int) 48 / TILE_HEIGHT; ti++)
    {
        ...
        for(int tj = 0; tj <= (int) 48  / TILE_WIDTH; tj++)
        {
            ...
            for(int kj = 0; kj <= (int) 96/(IN_BUF_DEPTH + 1); kj++)
            {
                ...
                // Depthwise weights are shaped [1][96][3][3], not [96][96][3][3].
                load_conv_layer_params_from_DRAM <1, 96, 3, 3>(conv_wt_buf, conv_bias_buf, layer_weights_2_1, layer_bias_2_1, 0, kj);
