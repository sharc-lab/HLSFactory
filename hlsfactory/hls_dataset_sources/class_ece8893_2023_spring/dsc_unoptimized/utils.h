...
)
{
    ...
    BIAS:
    if (out_channel == 1 && inp_channel > 1)
    {
        // Depthwise weights [1][C]: one bias per input channel (see d2conv_3x3).
        for (int c = 0; c < c_m; c++)
        {
            bias_buf[c] = bias[channel_offset + c];
        }
    }
    else
    {
        for (int f = 0; f < f_m; f++)
        {
            // TODO: assumes either b or d is 0 when both index terms are used.
            bias_buf[f] = bias[(kernel_offset + channel_offset) + f];
        }
    }
}
