...
void output_slector_single(
                  ...
                  bool direction,
                  VAR_TYPE_16 iter){
#pragma HLS disaggregate variable = fwd_valFromBU
#pragma HLS interface ap_fifo port = fwd_valFromBU._
#pragma HLS aggregate variable = fwd_valFromBU._ bit
#pragma HLS interface ap_fifo port = fwd_valFromBU._peek
#pragma HLS aggregate variable = fwd_valFromBU._peek bit
void(fwd_valFromBU._.empty());
void(fwd_valFromBU._peek.empty());

#pragma HLS disaggregate variable = inv_valFromLoad_0
#pragma HLS interface ap_fifo port = inv_valFromLoad_0._
#pragma HLS aggregate variable = inv_valFromLoad_0._ bit
#pragma HLS interface ap_fifo port = inv_valFromLoad_0._peek
#pragma HLS aggregate variable = inv_valFromLoad_0._peek bit
void(inv_valFromLoad_0._.empty());
void(inv_valFromLoad_0._peek.empty());

#pragma HLS disaggregate variable = inv_valFromShuf
#pragma HLS interface ap_fifo port = inv_valFromShuf._
#pragma HLS aggregate variable = inv_valFromShuf._ bit
#pragma HLS interface ap_fifo port = inv_valFromShuf._peek
#pragma HLS aggregate variable = inv_valFromShuf._peek bit
void(inv_valFromShuf._.empty());
void(inv_valFromShuf._peek.empty());

#pragma HLS disaggregate variable = fwd_valToShuf
#pragma HLS interface ap_fifo port = fwd_valToShuf._
#pragma HLS aggregate variable = fwd_valToShuf._ bit
void(fwd_valToShuf._.full());

#pragma HLS disaggregate variable = fwd_valToStore_0
#pragma HLS interface ap_fifo port = fwd_valToStore_0._
#pragma HLS aggregate variable = fwd_valToStore_0._ bit
void(fwd_valToStore_0._.full());

#pragma HLS disaggregate variable = inv_valToBU
#pragma HLS interface ap_fifo port = inv_valToBU._
#pragma HLS aggregate variable = inv_valToBU._ bit
void(inv_valToBU._.full());
  ...
  for(VAR_TYPE_16 iterCount=0; iterCount<(iter+1); iterCount++){
    for(VAR_TYPE_8 dataFlowIter=0; dataFlowIter<dataFlowIterLimit; dataFlowIter++){
      for(VAR_TYPE_16 i=0; i<dataCount;){
        #pragma HLS PIPELINE II = 1
        if(direction){
          inpStreamNotEmpty = !fwd_valFromBU.empty();

          if(inpStreamNotEmpty){
            if( dataFlowIter == (dataFlowIterLimit-1) ){
              fwd_valToStore_0.write(fwd_valFromBU.read());
            }
            else{
              fwd_valToShuf.write(fwd_valFromBU.read());
            }
            i++;
          }
        }
          ...
          if(dataFlowIter==0){
            inpStreamNotEmpty = !inv_valFromLoad_0.empty();
          }
          else{
            inpStreamNotEmpty = !inv_valFromShuf.empty();
          }

          if( inpStreamNotEmpty ){
            if(dataFlowIter==0){
              inv_valToBU.write(inv_valFromLoad_0.read());
            }
            else{
              inv_valToBU.write(inv_valFromShuf.read());
            }
            i++;
          }
        }
