...
#if defined(__SYNTHESIS__) || defined(HLSFACTORY_CSIM)
...
class istream {
 public:
  bool empty() const {
#pragma HLS inline
    // hls::stream::empty() is non-const in some Vitis releases (csim/clang).
    return const_cast<hls::stream<internal::elem_t<T>>&>(_).empty();
  }
...
class ostream {
 public:
  bool full() const {
#pragma HLS inline
    // hls::stream::full() is non-const in some Vitis releases (csim/clang).
    return const_cast<hls::stream<internal::elem_t<T>>&>(_).full();
  }
