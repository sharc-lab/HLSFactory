...
void  input_selector_single(
                    ...
                    bool direction,
                    VAR_TYPE_16 iter){
#pragma HLS disaggregate variable = fwd_valFromLoad_0
#pragma HLS interface ap_fifo port = fwd_valFromLoad_0._
#pragma HLS aggregate variable = fwd_valFromLoad_0._ bit
#pragma HLS interface ap_fifo port = fwd_valFromLoad_0._peek
#pragma HLS aggregate variable = fwd_valFromLoad_0._peek bit
void(fwd_valFromLoad_0._.empty());
void(fwd_valFromLoad_0._peek.empty());

#pragma HLS disaggregate variable = fwd_valFromShuf
#pragma HLS interface ap_fifo port = fwd_valFromShuf._
#pragma HLS aggregate variable = fwd_valFromShuf._ bit
#pragma HLS interface ap_fifo port = fwd_valFromShuf._peek
#pragma HLS aggregate variable = fwd_valFromShuf._peek bit
void(fwd_valFromShuf._.empty());
void(fwd_valFromShuf._peek.empty());

#pragma HLS disaggregate variable = inv_valFromBU
#pragma HLS interface ap_fifo port = inv_valFromBU._
#pragma HLS aggregate variable = inv_valFromBU._ bit
#pragma HLS interface ap_fifo port = inv_valFromBU._peek
#pragma HLS aggregate variable = inv_valFromBU._peek bit
void(inv_valFromBU._.empty());
void(inv_valFromBU._peek.empty());

#pragma HLS disaggregate variable = fwd_valToBU
#pragma HLS interface ap_fifo port = fwd_valToBU._
#pragma HLS aggregate variable = fwd_valToBU._ bit
void(fwd_valToBU._.full());

#pragma HLS disaggregate variable = inv_valToShuf
#pragma HLS interface ap_fifo port = inv_valToShuf._
#pragma HLS aggregate variable = inv_valToShuf._ bit
void(inv_valToShuf._.full());

#pragma HLS disaggregate variable = inv_valToStore_0
#pragma HLS interface ap_fifo port = inv_valToStore_0._
#pragma HLS aggregate variable = inv_valToStore_0._ bit
void(inv_valToStore_0._.full());
  ...
  for(VAR_TYPE_16 iterCount=0; iterCount<(iter+1); iterCount++){
    for(VAR_TYPE_8 dataFlowIter=0; dataFlowIter<dataFlowIterLimit; dataFlowIter++){
      for(VAR_TYPE_16 i=0; i<dataCount;){
        ...
        if(direction){
          if(dataFlowIter==0){
            inpStreamNotEmpty = !fwd_valFromLoad_0.empty();
          }
          else{
            inpStreamNotEmpty = !fwd_valFromShuf.empty();
          }
          if( inpStreamNotEmpty ){
            if(dataFlowIter==0){
              fwd_valToBU.write(fwd_valFromLoad_0.read());
            }
            else{
              fwd_valToBU.write(fwd_valFromShuf.read());
            }
            i++;
          }
        }
        else{
          inpStreamNotEmpty = !inv_valFromBU.empty();
          if(inpStreamNotEmpty){
            if( dataFlowIter == (dataFlowIterLimit-1) ){
              inv_valToStore_0.write(inv_valFromBU.read());
            }
            else{
              inv_valToShuf.write(inv_valFromBU.read());
            }
            i++;
          }
        }
