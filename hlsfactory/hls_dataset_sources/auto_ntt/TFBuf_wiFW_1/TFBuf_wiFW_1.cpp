...
void TFBuf_wiFW_1(
           ...
           VAR_TYPE_8 unit_id,
           VAR_TYPE_16 iter){
#pragma HLS disaggregate variable = loadTFStream
#pragma HLS interface ap_fifo port = loadTFStream._
#pragma HLS aggregate variable = loadTFStream._ bit
#pragma HLS interface ap_fifo port = loadTFStream._peek
#pragma HLS aggregate variable = loadTFStream._peek bit
void(loadTFStream._.empty());
void(loadTFStream._peek.empty());

#pragma HLS disaggregate variable = loadTFToNext
#pragma HLS interface ap_fifo port = loadTFToNext._
#pragma HLS aggregate variable = loadTFToNext._ bit
void(loadTFToNext._.full());

#pragma HLS disaggregate variable = tfToBu_0
#pragma HLS interface ap_fifo port = tfToBu_0._
#pragma HLS aggregate variable = tfToBu_0._ bit
void(tfToBu_0._.full());

#pragma HLS disaggregate variable = tfToBu_1
#pragma HLS interface ap_fifo port = tfToBu_1._
#pragma HLS aggregate variable = tfToBu_1._ bit
void(tfToBu_1._.full());
  ...
  const VAR_TYPE_16 TFArr_size = 138;
  const VAR_TYPE_16 fwd_tf_load_counter = 207;
  const VAR_TYPE_16 inv_tf_load_counter = 123;
  const VAR_TYPE_16 fwd_this_buf_limit = 69;
  const VAR_TYPE_16 inv_this_buf_limit = 41;

  const VAR_TYPE_16 fwd_log_stageWiseTFs[dataFlowIterLimit] = {1, 3, 7};
  const VAR_TYPE_16 inv_log_stageWiseTFs[dataFlowIterLimit] = {0, 4, 6};
  const VAR_TYPE_16 fwd_stageWiseOffset[dataFlowIterLimit] = {0, 2, 10};
  const VAR_TYPE_16 inv_stageWiseOffset[dataFlowIterLimit] = {0, 1, 17};
  const VAR_TYPE_16 fwd_log_num_val_accessed_by_consec_BUs_arr[dataFlowIterLimit] = {1, 1, 1};
  const VAR_TYPE_16 inv_log_num_val_accessed_by_consec_BUs_arr[dataFlowIterLimit] = {0, 0, 0};
  ...
  #pragma HLS bind_storage variable=TFArr type=RAM_T2P impl=bram
