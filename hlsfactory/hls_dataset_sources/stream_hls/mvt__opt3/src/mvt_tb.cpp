...
//   std::cout << "PRJ_PATH: " << prj_path_str << std::endl;
  ...
  if (input_0ifs.is_open()) {
    input_0ifs.read((char*)v0, 160000 * sizeof(float));
  } else {
  ...
  if (input_1ifs.is_open()) {
    input_1ifs.read((char*)v1, 160000 * sizeof(float));
  } else {
  ...
  if (input_2ifs.is_open()) {
    input_2ifs.read((char*)v2, 400 * sizeof(float));
  } else {
  ...
  if (input_3ifs.is_open()) {
    input_3ifs.read((char*)v3, 400 * sizeof(float));
  } else {
  ...
  if (output_0ifs.is_open()) {
    output_0ifs.read((char*)v7, 400 * sizeof(float));
  } else {
    std::cerr << "cannot open file /data/output_0.bin" << std::endl;
  }
  loop0: for (int v8 = 0; v8 < 400; v8++) {
    float v9 = v4[v8];
    float v10 = v7[v8];
    float v11 = v9 - v10;
    float v12 = v10 - v9;
  ...
  if (output_1ifs.is_open()) {
    output_1ifs.read((char*)v16, 400 * sizeof(float));
  } else {
    std::cerr << "cannot open file /data/output_1.bin" << std::endl;
  }
  loop1: for (int v17 = 0; v17 < 400; v17++) {
    float v18 = v5[v17];
    float v19 = v16[v17];
    float v20 = v18 - v19;
    float v21 = v19 - v18;
