...
//   std::cout << "PRJ_PATH: " << prj_path_str << std::endl;
  ...
  if (input_0ifs.is_open()) {
    input_0ifs.read((char*)v0, 37800 * sizeof(float));
  } else {
  ...
  if (input_1ifs.is_open()) {
    input_1ifs.read((char*)v1, 39900 * sizeof(float));
  } else {
  ...
  if (input_3ifs.is_open()) {
    input_3ifs.read((char*)v3, 39600 * sizeof(float));
  } else {
