...
int main() {
    ...
    fixed_t Q[B][N][dk];
    alignas(1024) fixed_t K[B][N][dk];
    alignas(1024) fixed_t V[B][N][dv];
    alignas(1024) fixed_t Output_ref[B][N][dv];
	alignas(1024) fixed_t Output_HLS[B][N][dv];
    ...
    // 3. (Optional) Print the first 512 bits from each loaded tensor
    {
        ap_uint<512> wide = *reinterpret_cast<ap_uint<512>*>(&Q[0][0][0]);
        std::string hexstr = wide.to_string(16, false);
        std::cout << "[Debug] Q's first 512 bits without alignas = 0x" << hexstr << std::endl;
    }
