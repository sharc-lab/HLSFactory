...
void csr_to_normal(data_t output[N][M], data_t values[], int column_indices[], int row_ptr[])
{
    int num_rows = N;
    int num_cols = M;
    ...
    for (int i = 0; i < num_rows; ++i) {
        for (int j = row_ptr[i]; j < row_ptr[i + 1]; ++j) {
            ...
            output[i][col] = values[j];     
        }
    }
}

void csc_to_normal(data_t output[M][K], data_t values[], int row_indices[], int column_ptr[])
{
    int num_rows = M;
    int num_cols = K;
    data_t temp_output[N][K];
    for (int i = 0; i < num_cols; ++i) {
        for (int j = column_ptr[i]; j < column_ptr[i + 1]; ++j) {
            int row = row_indices[j]; 
            temp_output[row][i] = values[j];     
        }
    }
    for (int i = 0; i < num_rows; ++i)
    {
