...
void compute_attention(fixed_t Q[B][N][dk], fixed_t K[B][N][dk], fixed_t V[B][N][dv], fixed_t Output[B][N][dv]) {
    ...
    #pragma HLS array_partition variable=Q dim=3 complete
    #pragma HLS array_partition variable=K dim=3 complete
    #pragma HLS array_partition variable=V dim=3 complete
    #pragma HLS array_partition variable=attention dim=3 complete
    ...
    for (int b = 0; b < B; ++b) {
        ...
        #pragma HLS pipeline II=1
            ...
            #pragma HLS unroll
                attention[b][i][j] = 0;
                for (int k = 0; k < dk; ++k) {
                    attention[b][i][j] += Q[b][i][k].to_float() * K[b][j][k].to_float();
                }
                attention[b][i][j] *= scale;
            }
        }
    }
    ...
    for (int b = 0; b < B; ++b) {
        ...
        #pragma HLS pipeline II=1
            ...
            #pragma HLS unroll
                Output[b][i][j] = 0;
                for (int k = 0; k < N; ++k) {
                    Output[b][i][j] += attention[b][i][k] * V[b][k][j].to_float();
                }
                Output[b][i][j] = (fixed_t)Output[b][i][j];
            }
        }
    }
}
