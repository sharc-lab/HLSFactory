...
void read_sparse_matrix_csr(const char *filename, data_t values[],
                            int column_indices[], int row_ptr[], int *nnz) {
...
void read_sparse_matrix_csc(const char *filename, data_t values[],
                            int row_indices[], int col_ptr[], int *nnz) {
  FILE *file = fopen(filename, "rb");
  if (!file) {
    perror("Failed to open file");
    exit(1);
...
void sparse_matrix_multiply(data_t values_A[], int column_indices_A[],
                            int row_ptr_A[], int nnz_A, data_t values_B[],
                            int row_indices_B[], int col_ptr_B[], int nnz_B,
                            float C[N][K]) {
...
int main() {
  ...
  char filename_A[50];
  snprintf(filename_A, sizeof(filename_A),
           "A_matrix_csr_sparsity_%.2f_%d_%d_%d.bin", SPARSITY, N, M, K);
  read_sparse_matrix_csr(filename_A, values_A, column_indices_A, row_ptr_A,
                         &nnz_A);

  char filename_B[50];
  snprintf(filename_B, sizeof(filename_B),
           "B_matrix_csc_sparsity_%.2f_%d_%d_%d.bin", SPARSITY, N, M, K);
  read_sparse_matrix_csc(filename_B, values_B, row_indices_B, col_ptr_B,
                         &nnz_B);

  char filename_C[50];
  snprintf(filename_C, sizeof(filename_B),
           "C_matrix_result_sparsity_%.2f_%d_%d_%d.bin", SPARSITY, N, M, K);
  ...
  sparse_matrix_multiply_HLS(values_A, column_indices_A, row_ptr_A, values_B,
                             row_indices_B, col_ptr_B, C_HLS);
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < K; j++) {
      ...
      if (C_HLS[i][j].to_float() != C_ref[i][j].to_float()) {
#if DEBUG == 1
        printf("(i,j) = (%d,%d) RefC: %.8f HLSC:%.8f\n", i, j,
