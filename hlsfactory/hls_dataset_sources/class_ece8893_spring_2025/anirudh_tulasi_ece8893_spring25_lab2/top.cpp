...
void compute_QK(fixed_t Q[B][N][dk], fixed_t K[B][N][dk], fixed_t attention[B][N][N]) {
...
void softmax_HLS(fixed_t matrix[B][N][N]) {
    #pragma HLS array_partition variable=matrix complete dim=3
    ...
    for (int b = 0; b < B; ++b) {
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < N; ++j) {
                #pragma HLS unroll factor=8
                temp_exp[j] = hls::exp(matrix[b][i][j] - max_val);
                sum += temp_exp[j];
            }

            for (int j = 0; j < N; ++j) {
                #pragma HLS unroll factor=8
                matrix[b][i][j] = temp_exp[j] / sum;
            }
        }
    }
}

// Compute Attention * V => Output
void compute_attentionV(fixed_t attention[B][N][N], fixed_t V[B][N][dv], fixed_t Output[B][N][dv]) {
...
void compute_attention_HLS(fixed_t Q[B][N][dk], fixed_t K[B][N][dk], fixed_t V[B][N][dv], fixed_t Output[B][N][dv]) 
{
    ...
    for (int b=0; b<B; b++)
    {
        for (int n=0; n<N; n++)
        {
            for (int i=0; i<dk; i++)
            {
                Q_local[b][n][i] = Q[b][n][i];
                K_local[b][n][i] = K[b][n][i];
            }

            for (int j=0; j<dv; j++)
            {
                V_local[b][n][j] = V[b][n][j];
            }
        }
    }

    compute_QK(Q_local, K_local, attention);

    softmax_HLS(attention);

    compute_attentionV(attention, V_local, Output);
}