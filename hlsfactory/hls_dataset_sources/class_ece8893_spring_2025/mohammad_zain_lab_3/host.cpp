...
int main() {
    ...
    int row_ptr_A[N + 1];
    int16_t row_ptr_A_reduced[N + 1];
    int nnz_A;

    // Matrix B (CSC format)
    data_t values_B[M * K];
    int row_indices_B[M * K];
    int16_t col_ptr_B_reduced[M * K];
    int col_ptr_B[M + 1];
    int nnz_B;
   ...
   // data_t C_HLS[N][K];
    ...
    //A to 2d vector
    hls::vector<data_t_reduced, N> values_A_vector[M];
    for(int i=0;i<M;i++){
        for(int j=0;j<N;j++){
            values_A_vector[i][j] = values_A[i*N+j];
        }
    }

    hls::vector<int16_t, N> column_indices_A_vector[M];
    for(int i=0;i<M;i++){
        for(int j=0;j<N;j++){
            column_indices_A_vector[i][j] = column_indices_A[i*N+j];
        }
    }
 // B to vector
    hls::vector<data_t_reduced, K> values_B_vector[M];
    for(int i=0;i<M;i++){
        for(int j=0;j<K;j++){
            values_B_vector[i][j] = values_B[i*K+j];
        }
    }

    //copy row_ptr_A to vector
    hls::vector<int16_t, N+1> row_ptr_A_vector;
    //define a vector of size N+1
    for(int i=0;i<N+1;i++){
        row_ptr_A_vector[i] = row_ptr_A[i];
    }

    hls::vector<int16_t, K> row_indices_B_vector[M];
    for(int i=0;i<M;i++){
        for(int j=0;j<K;j++){
            row_indices_B_vector[i][j] = row_indices_B[i*K+j];
        }
    }

    //copy to 16 bit A
    for(int i=0;i<N+1;i++){
        row_ptr_A_reduced[i] = row_ptr_A[i];
    }
    //copy to 16 bit B
    for(int i=0;i<M+1;i++){
        col_ptr_B_reduced[i] = col_ptr_B[i];
    }



    // Call HLS kernel to perform SpMM
    sparse_matrix_multiply_HLS(values_A_vector, column_indices_A_vector, row_ptr_A_reduced,
                           values_B_vector, row_indices_B_vector, col_ptr_B_reduced, C_HLS);
