...
/*
 *  Read Buffer position into JpegFileBuf
 *  (Vitis HLS does not support mutable global pointers)
 */
static int ReadBufPos;
...
read_word (void)
{
  ...
  c = JpegFileBuf[ReadBufPos++] << 8;
  c |= JpegFileBuf[ReadBufPos++];
...
get_sos ()
{
   ...
   */
  p_jinfo_jpeg_data_pos = ReadBufPos;
