...
for design_dir in design_dirs_new:
    ...
    lines = tcl_file.read_text().splitlines()
    # find the line that starts with set_part
    line_index = None
    for i, line in enumerate(lines):
        if line.strip().startswith("set_part"):
            line_index = i
            break
    if line_index is None:
        raise ValueError(f"Could not find line 'set_part' in {tcl_file}")
    
    lines[line_index] = "set_part {xczu9eg-ffvb1156-2-i}"
