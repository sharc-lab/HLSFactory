...
class DSEExplorer:
    ...
    def __init__(
        ...
        objective_fn: Optional[ObjectiveFunction] = None,
        target_ii: int = 1,
    ):
        """
    ...
    def find_affecting_pragmas(
        self, bottleneck_module: str, c_source_mappings: Dict
    ) -> List[int]:
        """
        ...
        if self.logger:
            self.logger.print(
                f"[DSEExplorer] Finding pragmas affecting module: {bottleneck_module}"
            )
        else:
            print(
                f"[DSEExplorer] Finding pragmas affecting module: {bottleneck_module}"
        ...
        if not module_c_info:
            if self.logger:
                self.logger.print(
                    f"[DSEExplorer] No C source mapping found for {bottleneck_module}, using random selection"
                )
            else:
                print(
                    f"[DSEExplorer] No C source mapping found for {bottleneck_module}, using random selection"
        ...
        start_line = module_c_info.get("start_line") or module_c_info.get(
            "start_lines", c_line
        )
        end_line = module_c_info.get("end_line") or module_c_info.get(
            "end_lines", c_line
        )
        ...
        if self.logger:
            self.logger.print(
                f"[DSEExplorer] Module {bottleneck_module} maps to function '{c_function}' at line {c_line} (range: {start_line}-{end_line}) in file '{c_file}'"
            )
        else:
            print(
                f"[DSEExplorer] Module {bottleneck_module} maps to function '{c_function}' at line {c_line} (range: {start_line}-{end_line}) in file '{c_file}'"
        ...
        for i, pragma_info in enumerate(self.pragma_locations):
            ...
            if c_file:
                ...
                file_matches = (
                    True  # No file restriction if mapping doesn't have file info
                )
            ...
            if file_matches:
                if match_all_in_file:
                    ...
                    if self.logger:
                        self.logger.print(
                            f"[DSEExplorer] ✓ Found affecting pragma #{i} (file match): {pragma_info['pragma_type']} at {pragma_file}:{pragma_line}"
                        )
                    ...
                    if start_line <= pragma_line <= end_line:
                        ...
                        if self.logger:
                            self.logger.print(
                                f"[DSEExplorer] ✓ Found affecting pragma #{i} (line match): {pragma_info['pragma_type']} at {pragma_file}:{pragma_line}"
                            )

        if not affecting_indices:
            if self.logger:
                self.logger.print(
                    "[DSEExplorer] No specific pragmas found, falling back to random selection"
                )
            else:
                print(
                    "[DSEExplorer] No specific pragmas found, falling back to random selection"
        ...
        if self.logger:
            self.logger.print(
                f"[DSEExplorer] Found {len(affecting_indices)} potentially affecting pragmas"
            )
        else:
            print(
                f"[DSEExplorer] Found {len(affecting_indices)} potentially affecting pragmas"
    ...
    def order_pragma_lists(self, affecting_indices: List[int]) -> List[Dict]:
        ...
        if self.logger:
            self.logger.print(
                f"[DSEExplorer] Ordered pragmas by priority: {[p['pragma_type'] for p in pragma_lists]}"
            )
        else:
            print(
                f"[DSEExplorer] Ordered pragmas by priority: {[p['pragma_type'] for p in pragma_lists]}"
    ...
    def order_pragma_lists_hierarchical(
        self,
        affecting_indices: List[int],
        csynth_data: Dict,
        rtl_data: Dict,
        work_dir: str,
        ...
        if self.logger:
            self.logger.print(
                "[DSEExplorer] Using hierarchical pragma ordering with loop hierarchy"
            )
        ...
        if self.logger:
            self.logger.print(
                f"[DSEExplorer] Loop pragma mapping: {loop_pragma_mapping}"
            )
        ...
        if "rtl_hierarchy" in rtl_data:
            ...
            def traverse_hierarchy(node, depth=0):
                ...
                children_with_lat = [
                    (c, module_latencies.get(c.get("ModuleName", ""), 0))
                    for c in children
                ]
        ...
        if self.logger:
            self.logger.print("[DSEExplorer] Module latency ranking (highest first):")
        ...
        for module_name, latency, loops in loop_modules:
            if self.logger:
                ...
                self.logger.print(
                    f"  Processing {module_name} (latency: {latency}) with loops: {loop_str}"
                )
            ...
            for loop_id in reversed(loops):
                if loop_id in loop_pragma_mapping and loop_id not in processed_loops:
                    self._add_children_first_then_parent(
                        loop_id,
                        loop_pragma_mapping,
                        processed_loops,
                        ordered_pragma_indices,
        ...
        if "top_module" in loop_pragma_mapping:
            ...
            if self.logger:
                self.logger.print(
                    f"    Added {len(loop_pragma_mapping['top_module'])} top module pragmas"
                )
                        ...
                        region_pragmas.append(
                            {
                                ...
                                "options": pragma_info["options"],
                                "region": region,
                                "priority": get_pragma_priority(
                                    pragma_info["pragma_type"]
                                ),
                            }
                        )
        ...
        if self.logger:
            self.logger.print("[DSEExplorer] Hierarchical pragma order (final):")
            for i, pragma in enumerate(final_ordered_pragmas):
                ...
                if pragma["index"] < len(self.pragma_locations):
                    ...
                    if region:
                        loop_info = f" ({region})"
                self.logger.print(
                    f"  {i + 1}. [{pragma['index']}] {pragma['pragma_type']}{loop_info}"
                )

        return final_ordered_pragmas

    def _add_children_first_then_parent(
        self,
        parent_loop: str,
        loop_pragma_mapping: Dict,
        processed_loops: set,
        ordered_pragma_indices: List[int],
        hierarchy_data: Dict,
        module_name: str,
    ):
        """
        ...
        if not hierarchy_data:
            ...
            if (
                parent_loop in loop_pragma_mapping
                and parent_loop not in processed_loops
            ):
                ordered_pragma_indices.extend(loop_pragma_mapping[parent_loop])
                processed_loops.add(parent_loop)
                if self.logger:
                    self.logger.print(
                        f"    Added pragmas for {parent_loop} from {module_name}"
                    )
            return
        ...
        if parent_loop in loop_pragma_mapping and parent_loop not in processed_loops:
            ...
            if self.logger:
                ...
                self.logger.print(
                    f"    Added pragmas for {parent_loop}{child_info} from {module_name}"
                )
    ...
    def _get_direct_children(self, parent_loop: str, hierarchy_data: Dict) -> List[str]:
        ...
        children = []
        self._find_children_recursive(
            hierarchy_data.get("children", []), parent_loop, children
        )
        return children

    def _find_children_recursive(
        self, children_nodes: List[Dict], target_loop: str, result: List[str]
    ):
        """
        ...
        for child in children_nodes:
            if child.get("type") == "loop":
                ...
                if loop_label == target_loop:
                    ...
                    self._find_children_recursive(
                        child.get("children", []), target_loop, result
                    )

    def _load_loop_hierarchy(
        self, work_dir: str, hierarchy_file_path: str = None
    ) -> Dict:
        """
        ...
        if hierarchy_file_path:
            try:
                ...
                with open(hierarchy_file_path, "r") as f:
                    ...
                    if self.logger:
                        self.logger.print(
                            f"[DSEExplorer] Loaded loop hierarchy from {hierarchy_file_path}"
                        )
                    return (
                        hierarchy_data[0]
                ...
                if self.logger:
                    self.logger.print(
                        f"[DSEExplorer] Failed to load hierarchy from {hierarchy_file_path}: {e}"
                    )
        ...
        if hierarchy_file.exists():
            try:
                ...
                with open(hierarchy_file, "r") as f:
                    ...
                    if self.logger:
                        self.logger.print(
                            f"[DSEExplorer] Loaded loop hierarchy from {hierarchy_file}"
                        )
                    return (
                        hierarchy_data[0]
                        if isinstance(hierarchy_data, list) and len(hierarchy_data) > 0
                ...
                if self.logger:
                    self.logger.print(
                        f"[DSEExplorer] Failed to parse hierarchy file {hierarchy_file}: {e}"
                    )

        # If no hierarchy file found, return empty structure
        if self.logger:
            self.logger.print(
                "[DSEExplorer] No loop hierarchy file found, using fallback ordering"
            )
        return {}

    def _order_loops_by_hierarchy(
        self, loop_parts: List[str], hierarchy_data: Dict
    ) -> List[str]:
        """
        ...
        if not hierarchy_data:
            ...
            return sorted(
                loop_parts, key=lambda x: int(x[1:]) if x[1:].isdigit() else 999
            )
        ...
        loop_parts_with_depth = [
            (loop, loop_to_depth.get(loop, 999)) for loop in loop_parts
        ]
        loop_parts_with_depth.sort(
            key=lambda x: x[1], reverse=True
        )  # Sort by depth descending (innermost first)

        return [loop for loop, depth in loop_parts_with_depth]

    def _add_loop_and_children_recursively(
        self,
        loop_id: str,
        loop_pragma_mapping: Dict,
        processed_loops: set,
        ...
        for child_loop in child_loops:
            if child_loop not in processed_loops:
                self._add_loop_and_children_recursively(
                    child_loop,
                    loop_pragma_mapping,
                    processed_loops,
                    ordered_pragma_indices,
    ...
    def _get_child_loops(self, parent_loop: str, hierarchy_data: Dict) -> List[str]:
        ...
        children = []
        self._find_loop_children(
            hierarchy_data.get("children", []), parent_loop, children
        )
        return children

    def _build_loop_depth_map(
        self, children: List[Dict], depth_map: Dict[str, int], current_depth: int
    ):
        """
        ...
        for child in children:
            if child.get("type") == "loop":
                ...
                self._build_loop_depth_map(
                    child.get("children", []), depth_map, current_depth + 1
                )

    def _find_loop_children(
        self, children: List[Dict], target_loop: str, result: List[str]
    ):
        """
    ...
    def _reset_focus(self, module: str, rtl_data: Dict):
        ...
        if self.logger:
            ...
            self.logger.print(
                f"[DFS Debug] Found {len(affecting)} affecting pragmas: {affecting}"
            )
            self.logger.print(
                f"[DFS Debug] Ordered pragmas: {[p['pragma_type'] for p in self.current_module_pragmas]}"
            )
    ...
    def _validate_resume_state(
        ...
        focus_pragma_idx: Optional[int] = None,
    ) -> bool:
        """
        ...
        for level, nodes in level_heap.items():
            if not isinstance(nodes, list):
                if self.logger:
                    self.logger.print(
                        f"❌ Resume validation failed: Level {level} nodes is not a list"
                    )
                return False
            ...
            for i, node in enumerate(nodes):
                for field in required_fields:
                    if field not in node:
                        if self.logger:
                            self.logger.print(
                                f"❌ Resume validation failed: Node at level {level}, index {i} missing field '{field}'"
                            )
                        return False
                ...
                if len(node["cfg"]) != len(self.pragma_locations):
                    if self.logger:
                        self.logger.print(
                            f"❌ Resume validation failed: Node at level {level}, index {i} has cfg length {len(node['cfg'])}, expected {len(self.pragma_locations)}"
                        )
                    return False
        ...
        if current_node is not None:
            ...
            for field in required_fields:
                if field not in current_node:
                    if self.logger:
                        self.logger.print(
                            f"❌ Resume validation failed: Current node missing field '{field}'"
                        )
                    return False
            ...
            if deepest_level is not None and deepest_level not in level_heap:
                if self.logger:
                    self.logger.print(
                        f"❌ Resume validation failed: Deepest level {deepest_level} not in heap (available levels: {list(level_heap.keys())})"
                    )
                return False

            # Validate focus_pragma_idx is valid
            if focus_pragma_idx is not None and focus_pragma_idx >= len(
                self.pragma_locations
            ):
                if self.logger:
                    self.logger.print(
                        f"❌ Resume validation failed: Focus pragma index {focus_pragma_idx} >= {len(self.pragma_locations)}"
                    )
                return False
    ...
    def _is_child_of(
        self, child_module: str, parent_module: str, rtl_data: Dict
    ) -> bool:
        """
        ...
        if not child_c_func or not parent_c_func:
            ...
            return (
                child_module.startswith(parent_module) and child_module != parent_module
            )
        ...
        is_child = (
            parent_c_func in child_c_func
            and child_c_func != parent_c_func
            and len(child_c_func) > len(parent_c_func)
        )

        if self.logger:
            self.logger.print(
                f"[DFS Debug] C function hierarchy check: {child_c_func} child of {parent_c_func}? {is_child}"
            )
    ...
    def _update_focus(self, new_module: str, rtl_data: Dict):
        ...
        if self.last_bottleneck_module is None:
            ...
            if self.logger:
                self.logger.print(
                    f"[DFS Debug] Initial focus setup for module: {new_module}"
                )
            ...
            if self.logger:
                self.logger.print(
                    f"[DFS Debug] Focus comparison: {new_module}({new_c_func}) vs {self.last_bottleneck_module}({self.last_bottleneck_c_function})"
                )
            ...
            if new_module == self.last_bottleneck_module:
                ...
                if self.logger:
                    self.logger.print(
                        "[DFS Debug] Same module - continuing current exploration"
                    )
                pass

            elif (
                new_c_func
                and self.last_bottleneck_c_function
                and new_c_func == self.last_bottleneck_c_function
            ):
                # Same C function - continue current exploration
                if self.logger:
                    self.logger.print(
                        "[DFS Debug] Same C function - continuing current exploration"
                    )
                pass
                ...
                if self.logger:
                    self.logger.print(
                        "[DFS Debug] Child relationship detected - refining focus"
                    )
                self._reset_focus(new_module, rtl_data)
                ...
                if self.logger:
                    self.logger.print(
                        "[DFS Debug] Different function/module - switching exploration branch"
                    )
                self._reset_focus(new_module, rtl_data)
        ...
        if changed_pragma_index is not None:
            ...
            if changed_pragma_index < len(self.pragma_locations):
                ...
                history_entry["changed_pragma_details"] = {
                    ...
                    "pragma_type": pragma_info["pragma_type"],
                    "category": pragma_info["category"],
                    "new_choice": choices[changed_pragma_index]
                    if changed_pragma_index < len(choices)
                    else "",
                }
    ...
    def _write_text_summary(self, output_dir: Path):
        ...
        with open(txt_file, "w") as f:
            ...
            if state_data:
                ...
                f.write(
                    f"📝 Objective Description: {state_data.get('objective_description', 'N/A')}\n"
                )
                ...
                if global_best_cfg:
                    f.write(
                        f"🏆 Global Best Configuration ({len(global_best_cfg)} pragmas):\n"
                    )
                ...
                if level_heap:
                    ...
                    for level in sorted(level_heap.keys()):
                        ...
                        for i, node in enumerate(nodes):
                            ...
                            f.write(
                                f"      Next Focus: {node.get('next_focus', 'N/A')}\n"
                            )
                            f.write(
                                f"      Tuned Pragmas: {len(node.get('tuned', []))} ({node.get('tuned', [])})\n"
                            )
                            f.write(
                                f"      Bottleneck Module: {node.get('bottleneck_module', 'N/A')}\n"
                            )
                ...
                if current_node is not None:
                    ...
                    f.write(
                        f"  Current Deepest Level: {state_data.get('current_deepest_level', 'N/A')}\n"
                    )
                    f.write(
                        f"  Current Focus Pragma Index: {state_data.get('current_focus_pragma_idx', 'N/A')}\n"
                    )

                    explored_options = state_data.get("explored_options", [])
                    f.write(
                        f"  Explored Options: {len(explored_options)} ({explored_options})\n"
                    )

                    f.write("  Current Node Details:\n")
                    f.write(f"    Score: {current_node.get('score', 'N/A')}\n")
                    f.write(
                        f"    Next Focus: {current_node.get('next_focus', 'N/A')}\n"
                    )
                    f.write(
                        f"    Tuned Pragmas: {len(current_node.get('tuned', []))} ({current_node.get('tuned', [])})\n"
                    )
                    f.write(
                        f"    Bottleneck Module: {current_node.get('bottleneck_module', 'N/A')}\n"
                    )
            ...
            for entry in self.history:
                f.write(
                    f"=== Iteration {entry['iter']} — {entry['status']} - new_launch {entry['new_launch']} ===\n"
                )
                ...
                if "affecting_pragmas_details" in entry:
                    f.write(
                        f"  📍 Pragmas of Interest ({len(entry['affecting_pragmas_details'])} found):\n"
                    )
                    for pragma in entry["affecting_pragmas_details"]:
                        f.write(
                            f"    [{pragma['index']}] {pragma['pragma_type']} in {pragma['file']}:{pragma['line']} -> '{pragma['current_choice']}'\n"
                        )

                if "changed_pragma_details" in entry:
                    changed = entry["changed_pragma_details"]
                    f.write(
                        f"  ✏️  Changed Pragma: [{changed['index']}] {changed['pragma_type']} in {changed['file']}:{changed['line']} -> '{changed['new_choice']}'\n"
                    )
                ...
                if "metrics" in entry:
                    ...
                    if "objective_result" in entry["metrics"]:
                        f.write(
                            f"  🎯 Objective Result: {entry['metrics']['objective_result']}\n"
                        )
                    ...
                    f.write(
                        "  📊 Metrics: " + json.dumps(entry["metrics"], indent=4) + "\n"
                    )
                ...
                for idx, choice in enumerate(entry["pragma_choices"]):
                    marker = (
                        " 👈 CHANGED"
                        if "changed_pragma_index" in entry
                        and entry["changed_pragma_index"] == idx
                        else ""
                    )
    ...
    def load_state(
        self, output_dir: str
    ) -> Tuple[int, List[str], Dict, List[str], int, Dict, int, int, List[str]]:
        """
        ...
        if checkpoint_file.exists():
            ...
            current_choices = state.get(
                "cur_pragmas", self.container.get_initial_choices()
            )
            ...
            if self.logger:
                ...
                if level_heap:
                    self.logger.print(
                        f"[DSEExplorer] Loaded LevelHeap with {len(level_heap)} levels"
                    )
                    for level in sorted(level_heap.keys()):
                        self.logger.print(
                            f"  Level {level}: {len(level_heap[level])} nodes"
                        )
                if global_best_cfg is not None:
                    self.logger.print(
                        f"[DSEExplorer] Global best score: {global_best_score}"
                    )
                if current_node is not None:
                    self.logger.print("[DSEExplorer] Resuming mid-node expansion:")
            ...
            return (
                0,
                self.container.get_initial_choices(),
                {},
                None,
                "FAIL SYNTHESIS",
                None,
    ...
    def run_exploration_LevelHeap_DFS(
        self,
        max_iterations: int,
        output_dir: str,
        timeout_min: int = 10,
        max_exploration_hours: Optional[float] = None,
    ) -> List[Dict]:
        """
        ...
        max_exploration_seconds = (
            max_exploration_hours * 3600 if max_exploration_hours else None
        )

        if self.logger:
            if max_exploration_hours:
                self.logger.print(
                    f"[DSEExplorer] Starting LevelHeap DFS exploration: {max_exploration_hours} hours, timeout {timeout_min}min"
                )
            else:
                self.logger.print(
                    f"[DSEExplorer] Starting LevelHeap DFS exploration: {max_iterations} iterations, timeout {timeout_min}min"
                )
        else:
            if max_exploration_hours:
                print(
        ...
        (
            ...
            loaded_level_heap,
            loaded_global_best_cfg,
            loaded_global_best_score,
            loaded_current_node,
            loaded_deepest_level,
            loaded_focus_pragma_idx,
            loaded_explored_options,
        ) = self.load_state(output_dir)
        ...
        level_heap = (
            loaded_level_heap if loaded_level_heap else {}
        )  # level -> list of nodes
        global_best_cfg = loaded_global_best_cfg if loaded_global_best_cfg else None
        global_best_score = (
            loaded_global_best_score
            if loaded_global_best_score != "FAIL SYNTHESIS"
            else "FAIL SYNTHESIS"
        ...
        resume_explored_options = (
            loaded_explored_options if loaded_explored_options else []
        )
        ...
        if start_iter == 0:
            ...
            if self.logger:
                ...
                self.logger.print(
                    f"[DSEExplorer] Set {unroll_count} unroll pragmas to 1 for initial configuration"
                )
            ...
            variant_dir = self.container.generate_variant(
                self.current_choices, iteration, output_dir
            )
            ...
            if result_status == "success":
                ...
                if csynth_file.exists() and rtl_file.exists():
                    ...
                    objective_result, objective_details = self.objective_fn.evaluate(
                        csynth_data, rtl_data
                    )
                    bottleneck_module = objective_details.get(
                    ...
                    if objective_result == ObjectiveResult.INFEASIBLE:
                        ...
                        if self.logger:
                            violations = objective_details.get(
                                "resource_violations", []
                            )
                            self.logger.print(
                                f"❌ Default configuration exceeds resource constraints: {violations}"
                            )
                    else:
                        # Extract score based on objective type
                        if hasattr(objective_details, "get"):
                            raw_score = objective_details.get(
                                "latency",
                                objective_details.get(
                                    "current_ii",
                                    objective_details.get("combined_score", 1000),
                                ),
                            )
                            try:
                    ...
                    affecting_indices = self.find_affecting_pragmas(
                        bottleneck_module, rtl_data
                    )
                    ...
                    ordered_pragmas = self.order_pragma_lists_hierarchical(
                        all_pragma_indices, csynth_data, rtl_data, work_dir
                    )
                    ...
                    if self.logger:
                        self.logger.print(
                            f"✅ Default config evaluated: score={score}, bottleneck={bottleneck_module}"
                        )
                        self.logger.print(
                            "[LevelHeap] Initialized Level 0 with default config"
                        )
                        self.logger.print(
                            f"[LevelHeap] Next focus pragma index: {next_focus}"
                        )
                    ...
                    enhanced_metrics["objective_result"] = (
                        objective_result.value
                        if hasattr(objective_result, "value")
                        else str(objective_result)
                    )
                ...
                if self.logger:
                    self.logger.print(
                        f"❌ Default configuration synthesis failed: {result_status}"
                    )
                return self.history
        else:
            ...
            if self.logger:
                ...
                self.logger.print(
                    f"[LevelHeap] Loaded heap state: {len(level_heap)} levels"
                )
                for level in sorted(level_heap.keys()):
                    self.logger.print(
                        f"  Level {level}: {len(level_heap[level])} nodes"
            ...
            if resume_current_node is not None:
                if self.logger:
                    self.logger.print(
                        f"[LevelHeap] Will resume mid-node expansion at level {resume_deepest_level}"
                    )
                    self.logger.print(
                        f"[LevelHeap] Will skip {len(resume_explored_options)} already-explored options"
                    )
            ...
            if not self._validate_resume_state(level_heap, None, None, None):
                if self.logger:
                    self.logger.print(
                        "❌ ERROR: Resume state validation failed, cannot resume"
                    )
                return self.history
        ...
        completed_iterations = len(
            [
                ...
                if h.get("status")
                not in ["iteration_limit_reached", "time_limit_reached"]
            ]
        )

        if self.logger:
            self.logger.print(
                f"[DEBUG] Starting main loop: iteration={iteration}, max_iterations={max_iterations}"
            )
            self.logger.print(
                f"[DEBUG] Completed iterations so far: {completed_iterations}"
            )
            self.logger.print(
                f"[DEBUG] Initial level_heap: {list(level_heap.keys()) if level_heap else 'empty'}"
            )
            self.logger.print(
                f"[DEBUG] Iterations remaining: {max(0, max_iterations - completed_iterations)}"
            )
        ...
        while True:
            ...
            if max_exploration_seconds:
                ...
                if elapsed_time >= max_exploration_seconds:
                    if self.logger:
                        self.logger.print(
                            f"⏰ Time limit reached: {elapsed_time / 3600:.2f} hours"
                        )
            ...
            if completed_iterations >= max_iterations:
                if self.logger:
                    self.logger.print(
                        f"🔢 Iteration limit reached: {completed_iterations} completed iterations >= {max_iterations} max"
                    )
                    self.logger.print(
                        "   Note: max_iterations is the TOTAL iteration count (including resumed runs)"
                    )
                    self.logger.print(
                        f"   To run more iterations, increase max_iterations to > {completed_iterations}"
                    )
                should_continue = False
            ...
            if not level_heap:
                if self.logger:
                    self.logger.print(
                        "[LevelHeap] No more nodes to explore - heap is empty"
                    )
                    self.logger.print(
                        f"[LevelHeap] Exploration ended after {iteration - start_iter} iterations"
                    )
                should_continue = False
            ...
            if max_exploration_seconds:
                ...
                if self.logger:
                    ...
                    self.logger.print(
                        f"⏰ Time: {elapsed_time / 3600:.2f}h elapsed, {remaining_time / 3600:.2f}h remaining"
                    )
                    self.logger.print(
                        f"[LevelHeap] Current heap levels: {list(level_heap.keys())}"
                    )
                else:
                    print(f"\n=== LevelHeap DFS Iteration {iteration} ===")
                    print(
            ...
            if resume_current_node is not None:
                ...
                if self.logger:
                    self.logger.print(
                        f"[LevelHeap] Resuming expansion of saved node at level {deepest_level}"
                    )
                    self.logger.print(
                        f"[LevelHeap] Current node: tuned={current_node['tuned']}, next_focus={current_node['next_focus']}, score={current_node['score']}"
                    )
                ...
                if self.logger:
                    self.logger.print(
                        f"[LevelHeap] Exploring BEST node from level {deepest_level} (selected {best_node_idx + 1}/{len(current_level_nodes) + 1})"
                    )
                    self.logger.print(
                        f"[LevelHeap] Current node: tuned={current_node['tuned']}, next_focus={current_node['next_focus']}, score={current_node['score']}"
                    )
            ...
            if self.logger:
                self.logger.log_node_expansion_start(
                    iteration,
                    deepest_level,
                    current_node,
                    self.pragma_locations,
            ...
            if focus_pragma_idx >= len(self.pragma_locations):
                if self.logger:
                    self.logger.print(
                        f"[LevelHeap] Invalid pragma index {focus_pragma_idx}, continuing"
                    )
                continue
            ...
            if self.logger:
                self.logger.print(
                    f"[LevelHeap] Expanding pragma {focus_pragma_idx} ({pragma_info['pragma_type']}) with {len(pragma_options)} options"
                )

            # Check if we're resuming this specific node's expansion
            # If so, we need to track which options have been explored
            if (
                resume_current_node is not None
                and resume_deepest_level == deepest_level
                and resume_focus_pragma_idx == focus_pragma_idx
                and current_node == resume_current_node
            ):
                ...
                if self.logger:
                    self.logger.print(
                        f"[LevelHeap] Resuming node expansion, will skip {len(explored_options_for_this_node)} options: {explored_options_for_this_node}"
                    )
            ...
            for option in pragma_options:
                ...
                if max_exploration_seconds:
                    ...
                    if elapsed_time >= max_exploration_seconds:
                        if self.logger:
                            self.logger.print(
                                "⏰ Time limit reached during pragma option exploration"
                            )
                        ...
                        if explored_options_for_this_node:
                            self.save_state(
                                ...
                                iteration,
                                new_cfg
                                if "new_cfg" in locals()
                                else current_node["cfg"],
                                "time_limit_reached",
                                {},
                                None,
                                [],
                ...
                if option in explored_options_for_this_node:
                    if self.logger:
                        self.logger.print(
                            f"[LevelHeap] Skipping already-explored option: '{option}'"
                        )
                    continue
                ...
                if self.logger:
                    self.logger.print(
                        f"[LevelHeap] Trying pragma {focus_pragma_idx}: '{old_value}' -> '{option}'"
                    )
                ...
                if skip_synthesis:
                    if self.logger:
                        self.logger.print(
                            f"[LevelHeap] Option '{option}' same as current value - reusing parent node results, skipping synthesis"
                        )
                ...
                if skip_synthesis:
                    ...
                    if self._is_valid_score(score):
                        ...
                        tuned_pragmas = set(
                            [pragma_idx for pragma_idx, _ in current_node["tuned"]]
                            + [focus_pragma_idx]
                        )
                        next_focus = None
                        for pragma_dict in ordered_pragmas:
                        ...
                        if self.logger:
                            self.logger.print(
                                f"✅ Added new node to level {next_level}: score={score}, next_focus={next_focus} (reused parent data)"
                            )
                    ...
                    try:
                        ...
                        if self.logger:
                            ...
                            self.logger.print(
                                "   Marking design as unanalyzable and continuing DSE"
                            )
                    ...
                    if analysis_success and csynth_file.exists() and rtl_file.exists():
                        ...
                        # Evaluate objective function
                        objective_result, objective_details = (
                            self.objective_fn.evaluate(csynth_data, rtl_data)
                        )
                        bottleneck_module = objective_details.get(
                            "bottleneck_module", "FFT_TOP"
                        )
                        ...
                        if objective_result == ObjectiveResult.INFEASIBLE:
                            # Design failed due to resource constraints
                            score = self._get_failure_score(
                                result_status, objective_result
                            )
                            if self.logger:
                                violations = objective_details.get(
                                    "resource_violations", []
                                ...
                                self.logger.print(
                                    f"❌ Resource constraints violated: {violations}"
                                )
                        else:
                            # Extract score based on objective type
                            raw_score = objective_details.get(
                        ...
                        if self._is_valid_score(score):
                            ...
                            # Create new node for next level
                            new_tuned = current_node["tuned"] + [
                                (focus_pragma_idx, option)
                            ]
                            new_node = {
                                "cfg": new_cfg,
                                "tuned": new_tuned,
                            ...
                            if self.logger:
                                self.logger.print(
                                    f"✅ Added new node to level {next_level}: score={score}, next_focus={next_focus}"
                                )
                                # Log child node generation
                                self.logger.log_child_node_generation(
                                    iteration,
                                    focus_pragma_idx,
                                    old_value,
                                    option,
                            ...
                            if self.logger:
                                self.logger.print(
                                    f"🚫 Pruning node with invalid score: {score}, will not expand further"
                                )
                        ...
                        if focus_pragma_idx < len(self.pragma_locations):
                            ...
                            changed_pragma_details = {
                                ...
                                "file": pragma_info["file"],
                                "line": pragma_info["line"],
                                "new_choice": new_cfg[focus_pragma_idx]
                                if focus_pragma_idx < len(new_cfg)
                                else "",
                            }
                        ...
                        if not analysis_success:
                            ...
                            if self.logger:
                                self.logger.print(
                                    f"⚠️  Synthesis succeeded but analysis failed for iteration {iteration}"
                                )
                        else:
                            ...
                            if self.logger:
                                self.logger.print(
                                    f"❌ Missing analysis files for iteration {iteration}"
                                )
                        ...
                        if self.logger:
                            self.logger.print(
                                f"🚫 Pruning failed analysis node: score={score}, will not expand further"
                            )
                    ...
                    if self.logger:
                        self.logger.print(
                            f"🚫 Pruning failed synthesis node: score={score}, will not expand further"
                        )
                ...
                if not skip_synthesis:
                    ...
                    if self.logger:
                        self.logger.print(
                            f"[DEBUG] Incremented iteration to {iteration}, completed_iterations={completed_iterations}"
                        )
                else:
                    if self.logger:
                        self.logger.print(
                            f"[DEBUG] Incremented iteration to {iteration}, completed_iterations unchanged (synthesis skipped)"
                        )
            ...
            remaining_options = [
                opt
                for opt in pragma_options
                if opt not in explored_options_for_this_node
            ]

            if self.logger:
                if not remaining_options:
                    self.logger.print(
                        "[LevelHeap] All options explored for this node - node expansion complete"
                    )
                elif explored_options_for_this_node:
                    self.logger.print(
                        f"[LevelHeap] Explored {len(explored_options_for_this_node)} option(s), continuing DFS with children"
                    )
                else:
                    self.logger.print(
                        "[LevelHeap] No options were explored this iteration"
                    )
        ...
        if self.logger:
            ...
            self.logger.print(
                f"[LevelHeap DFS] Total exploration time: {total_exploration_time / 3600:.2f} hours"
            )
            ...
            self.logger.print(
                f"[LevelHeap DFS] Final heap state: {len(level_heap)} levels"
            )
            for level, nodes in level_heap.items():
