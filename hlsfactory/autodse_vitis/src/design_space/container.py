...
class TemplateContainer:
    ...
    def load_template(self) -> Dict:
        ...
        if self.logger:
            self.logger.print(
                f"[TemplateContainer] Loading template from {self.template_dir}"
            )
        else:
        ...
        if self.logger:
            ...
            self.logger.print(
                f"[TemplateContainer] Found {len(self.pragma_locations)} pragma locations"
            )
            self.logger.print(
                f"[TemplateContainer] Pragma types: {list(self.pragma_options.keys())}"
            )
        else:
    ...
    def _parse_hls_auto_pragmas(self):
        ...
        for filename, content in self.template_files.items():
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if match:
                    pragma_type = match.group(
                        1
                    )  # e.g., "dataflow", "inline", "performance"
                    pragma_params = match.group(2).strip()  # e.g., "target_ti=", ""
                    options_str = match.group(
                        3
                    )  # e.g., "NULL, disable_start_propagation"
    ...
    def _load_json_options(self):
        ...
        if not json_files:
            raise FileNotFoundError(
                f"JSON format detected but no JSON file found in {self.template_dir}"
            )
        ...
        if self.logger:
            self.logger.print(
                f"[TemplateContainer] Loaded {len(self.json_options)} pragma option sets from {json_file.name}"
            )
        else:
            print(
                f"[TemplateContainer] Loaded {len(self.json_options)} pragma option sets from {json_file.name}"
    ...
    def _parse_json_format_pragmas(self):
        ...
        for pragma_type in self.pragma_options:
            if isinstance(self.pragma_options[pragma_type], set):
                self.pragma_options[pragma_type] = list(
                    self.pragma_options[pragma_type]
                )
    ...
    def generate_variant(
        self, pragma_choices: List[str], variant_id: int, output_dir: str
    ) -> str:
        """
        ...
        if self.logger:
            self.logger.print(
                f"[TemplateContainer] Generating variant {variant_id} in {variant_dir}"
            )
        else:
            print(
                f"[TemplateContainer] Generating variant {variant_id} in {variant_dir}"
        ...
        for filename, original_content in self.template_files.items():
            modified_content = self._apply_pragma_choices(
                filename, original_content, pragma_choices
            )
        ...
        if self.logger:
            self.logger.print(
                f"[TemplateContainer] Variant {variant_id} generated successfully"
            )
        else:
            print(f"[TemplateContainer] Variant {variant_id} generated successfully")
        return str(variant_dir)

    def _apply_pragma_choices(
        self, filename: str, content: str, pragma_choices: List[str]
    ) -> str:
        """
