...
class TeeLogger:
    ...
    def save_dse_trace(
        ...
        affecting_pragmas: List[Dict],
        changed_pragma: Optional[Dict],
        pragma_locations: List[Dict],
        ...
        with open(self.dse_trace_file, "a") as f:
            ...
            if csynth_file.exists() and rtl_file.exists():
                try:
                    ...
                    if bottleneck_module in modules_performance:
                        ...
                        f.write(
                            f"Bottleneck module: {bottleneck_module} (Latency={latency}, II={ii})\n"
                        )

                        # Get C source info
                        c_info = c_mappings.get(bottleneck_module, {}).get(
                            "c_source", {}
                        )
                        mapping = c_info.get("mapping_type", "?")
                        if (
                            c_info.get("c_loops")
                            and isinstance(c_info["c_loops"], list)
                            and len(c_info["c_loops"]) > 0
                        ):
                            ...
                            c_name = c_info.get(
                                "c_name", c_info.get("llvm_module", "unknown")
                            )
                        cline = c_info.get("c_line")
                        extra = f", line {cline}" if cline else ""
                        f.write(
                            f"Bottleneck module → C {c_type}: {c_name} (mapping={mapping}{extra})\n"
                        )
                    else:
                        f.write(
                            f"Bottleneck module: {bottleneck_module} (metrics not available)\n"
                        )
            ...
            if affecting_pragmas:
                for pragma in affecting_pragmas:
                    f.write(
                        f"  {pragma['pragma_type']} in {pragma['file']}:{pragma['line']} -> '{pragma['current_choice']}'\n"
                    )
            else:
                f.write("  (none found)\n")
            ...
            if changed_pragma:
                f.write(
                    f"  {changed_pragma['pragma_type']} in {changed_pragma['file']}:{changed_pragma['line']} -> '{changed_pragma['new_choice']}'\n"
                )
            else:
                f.write("  (none)\n")
    ...
    def log_node_expansion_start(
        self,
        iteration: int,
        deepest_level: int,
        current_node: Dict,
        pragma_locations: List[Dict],
        focus_pragma_idx: int,
    ):
        ...
        with open(self.levelheap_trace_file, "a") as f:
            ...
            f.write(
                f"[EXPANSION] Selected node from Level {deepest_level} for expansion:\n"
            )
            ...
            if focus_pragma_idx < len(pragma_locations):
                ...
                f.write(
                    f" ({pragma_info['pragma_type']}@{pragma_info['file']}:{pragma_info['line']}, {region})"
                )
                f.write(f"\n[EXPANSION]   Focus pragma options: {options}\n")
            else:
                f.write(" (invalid index)\n")

            f.write(
                f"[EXPANSION]   Bottleneck module: {current_node.get('bottleneck_module', 'N/A')}\n"
            )
            f.write("\n")

    def log_child_node_generation(
        self,
        iteration: int,
        focus_pragma_idx: int,
        old_value: str,
        ...
        with open(self.levelheap_trace_file, "a") as f:
            f.write(
                f"[EXPANSION] Child node generated for pragma {focus_pragma_idx}: '{old_value}' -> '{new_value}'\n"
            )
            ...
            if next_focus is not None and next_focus < len(pragma_locations):
                ...
                region = pragma_info.get("region", "unknown")
                f.write(
                    f"[EXPANSION]   Next focus for child: {next_focus} ({pragma_info['pragma_type']}@{pragma_info['file']}:{pragma_info['line']}, {region})\n"
                )
            else:
                f.write(
                    f"[EXPANSION]   Next focus for child: {next_focus} (end of exploration)\n"
                )
            f.write("\n")
        ...
        with open(self.levelheap_trace_file, "a") as f:
                        ...
                        for i, node in enumerate(nodes):
                            ...
                            for idx, val in node.get("tuned", []):
                                if idx < len(pragma_locations):
                                    ...
                                    tuned_details.append(f"{idx}={val}")
                            tuned_str = (
                                ", ".join(tuned_details) if tuned_details else "none"
                            )

                            # Format next focus with detailed info
                            next_focus = node.get("next_focus", None)
                            if next_focus is not None and next_focus < len(
                                pragma_locations
                            ):
                                pragma_info = pragma_locations[next_focus]
            ...
            if current_node:
                ...
                if next_focus is not None and next_focus < len(pragma_locations):
                    ...
                    next_focus_str = (
                        str(next_focus) if next_focus is not None else "None"
                    )

                f.write(
                    f"  Score: {self._format_score(current_node.get('score', 'N/A'))}\n"
                )
                f.write(f"  Tuned pragmas: [{tuned_str}]\n")
                f.write(f"  Next focus: {next_focus_str}\n")
                f.write(
            ...
            if current_node and "ordered_pragmas" in current_node:
                ...
                for i, pragma_dict in enumerate(current_node["ordered_pragmas"]):
                    ...
                    current_choice = (
                        current_choices[pragma_idx]
                        if pragma_idx < len(current_choices)
                        else "N/A"
                    )
                    tuned_mark = (
                        " (TUNED)"
                        if any(
                            idx == pragma_idx
                            for idx, _ in current_node.get("tuned", [])
                        )
                        else ""
                    )
                    if pragma_idx < len(pragma_locations):
                        ...
                        region = pragma_info.get("region", "unknown")
                        location = (
                            f"@{pragma_info['file']}:{pragma_info['line']}, {region}"
                        )
                    else:
                        location = ""
                    f.write(
                        f"  {i + 1}. [{pragma_idx}] {pragma_type}{location} -> '{current_choice}'{tuned_mark}\n"
                    )
    ...
    def _initialize_levelheap_trace(self, work_dir: str):
        ...
        # Check if files already exist (resuming case)
        trace_exists = self.levelheap_trace_file.exists()
        final_trace_json = trace_dir / "final_trace.json"
        final_trace_exists = final_trace_json.exists()

        # Initialize final_trace.json (will skip if already exists)
        self._initialize_final_trace_json(trace_dir)

        try:
            if trace_exists:
                # Resuming - just mark as initialized and append future entries
                self.levelheap_trace_initialized = True
                self.print(
                    f"[TeeLogger] LevelHeap trace resumed (existing file): {self.levelheap_trace_file}"
                )
                if final_trace_exists:
                    self.print(
                        f"[TeeLogger] Final trace JSON resumed (existing file): {final_trace_json}"
                    )
            else:
                ...
                self.levelheap_trace_initialized = True
                self.print(
                    f"[TeeLogger] LevelHeap trace initialized: {self.levelheap_trace_file}"
                )
    ...
    def _initialize_final_trace_json(self, trace_dir: Path):
        ...
        try:
            # Check if file already exists (resuming case)
            if self.final_trace_json_file.exists():
                # Resuming - validate the file can be read
                try:
                    with open(self.final_trace_json_file, "r") as f:
                        existing_data = json.load(f)
                    self.final_trace_json_initialized = True
                    self.print(
                        f"[TeeLogger] Final trace JSON resumed with {len(existing_data)} existing entries: {self.final_trace_json_file}"
                    )
                except json.JSONDecodeError as e:
                    self.print(
                        f"[TeeLogger] Warning: Existing final_trace.json is corrupted, recreating: {e}"
                    )
                    # Backup the corrupted file
                    backup_file = self.final_trace_json_file.with_suffix(".json.backup")
                    self.final_trace_json_file.rename(backup_file)
                    self.print(
                        f"[TeeLogger] Backed up corrupted file to: {backup_file}"
                    )
                    # Create new file
                    ...
                    self.final_trace_json_initialized = True
                    self.print(
                        f"[TeeLogger] Final trace JSON recreated: {self.final_trace_json_file}"
                    )
            else:
                # Fresh start - initialize with empty JSON object
                with open(self.final_trace_json_file, "w") as f:
                    json.dump({}, f, indent=2)

                self.final_trace_json_initialized = True
                self.print(
                    f"[TeeLogger] Final trace JSON initialized: {self.final_trace_json_file}"
                )
    ...
    def update_levelheap_trace_input_hierarchy(self, work_dir: str):
        ...
        if input_c_file.exists():
            ...
            if "(Input C hierarchy not available yet" in content:
                ...
                self.print("[TeeLogger] Updated LevelHeap trace with input C hierarchy")
    ...
    def _write_module_hierarchy_table(self, f, csynth_data: Dict, rtl_data: Dict):
        ...
        self._collect_hierarchy_data(
            hierarchy_tree, modules_performance, c_mappings, 0, table_data
        )
    ...
    def _collect_hierarchy_data(
        ...
        for child in node.get("Children", []):
            self._collect_hierarchy_data(
                child, modules_performance, c_mappings, depth + 1, table_data
            )
    ...
    def _write_table_row(
        self, f, row_data: List[str], col_widths: List[int], is_header: bool = False
    ):
    ...
    def save_iteration_trace(
        ...
        pragma_groups = {"__PARA__": [], "__PIPE__": [], "__TILE__": [], "__DATA__": []}
            ...
            if i < len(pragma_locations):
                ...
                if group_key == "__PIPE__":
                    ...
                    if choice and choice.isdigit():
                        ...
                        point_dict[point_key] = (
                            choice  # Keep non-numeric choices as string
                        )
                    else:
        ...
        if synthesis_status == "success" and obj_details:
            ...
            latency_raw = obj_details.get("latency", 0.0)
            try:
                perf_value = float(latency_raw)
            ...
            valid = synthesis_status == "success" and not obj_details.get(
                "resource_violations", []
            )
