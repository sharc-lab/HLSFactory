...
class ObjectiveFunction(ABC):
    ...
    @abstractmethod
    def evaluate(
        self, metrics: Dict[str, Any], rtl_data: Dict[str, Any]
    ) -> Tuple[ObjectiveResult, Dict[str, Any]]:
        """
...
class TargetIIObjective(ObjectiveFunction):
    ...
    def evaluate(
        ...
        details = {"target_ii": self.target_ii, "top_module": self.top_module}
        ...
        if "metrics" in metrics and self.top_module in metrics["metrics"]:
            ...
            if ii_value is None or (
                isinstance(ii_value, str) and ii_value.lower() in ("undef", "-", "")
            ):
                current_ii = float("inf")
            else:
                try:
                    current_ii = (
                        int(ii_value) if not isinstance(ii_value, float) else ii_value
                    )
                except (ValueError, TypeError):
                    current_ii = float("inf")
            ...
            if current_ii <= self.target_ii:
                ...
                details["bottleneck_module"] = self._find_bottleneck_module(
                    metrics, rtl_data
                )
    ...
    def _find_bottleneck_module(
        self, metrics: Dict[str, Any], rtl_data: Dict[str, Any]
    ) -> str:
        """Find the bottleneck module using existing DSE logic."""
        self._log(
            f"[TargetIIObjective] Finding bottleneck with target II = {self.target_ii}"
        )
        ...
        bottleneck_module = self._find_worst_module_recursive(
            hierarchy_tree, modules_performance, self.target_ii
        )

        self._log(
            f"[TargetIIObjective] Identified bottleneck module: {bottleneck_module}"
        )
        return bottleneck_module

    def _find_worst_module_recursive(
        self, hierarchy_node: Dict, modules_performance: Dict, target_ii: int
    ) -> str:
        ...
        if module_name in modules_performance:
            ii_value = (
                ...
                .get("performance", {})
                .get("PipelineInitiationInterval", 0)
            )
            current_ii = self._safe_int(ii_value, 0)

        self._log(
            f"[TargetIIObjective] Checking module {module_name}: II = {current_ii}"
        )

        # Check if current module meets target
        if current_ii <= target_ii:
            self._log(
                f"[TargetIIObjective] Module {module_name} meets target II ({current_ii} <= {target_ii})"
            )
        ...
        if not children:
            ...
            self._log(
                f"[TargetIIObjective] Leaf bottleneck found: {module_name} (II = {current_ii})"
            )
            return module_name
        ...
        for child_node in children:
            ...
            if child_name in modules_performance:
                ii_value = (
                    modules_performance[child_name]
                    .get("performance", {})
                    .get("PipelineInitiationInterval", 0)
                )
        ...
        if all_children_meet_target and current_ii > target_ii:
            self._log(
                f"[TargetIIObjective] Parent module {module_name} doesn't meet target but all children do - returning parent as bottleneck"
            )
            return module_name

        if worst_child_node:
            # Recursively examine the worst child
            return self._find_worst_module_recursive(
                worst_child_node, modules_performance, target_ii
            )
        else:
...
class ResourceConstrainedLatencyObjective(ObjectiveFunction):
    ...
    def evaluate(
        ...
        if "metrics" in metrics and self.top_module in metrics["metrics"]:
            ...
            if resource_violations:
                ...
                details["bottleneck_module"] = self._find_resource_bottleneck(
                    metrics, rtl_data
                )
                return ObjectiveResult.INFEASIBLE, details
            else:
                # Resources OK, focus on latency improvement
                details["bottleneck_module"] = self._find_latency_bottleneck(
                    metrics, rtl_data
                )
    ...
    def _find_latency_bottleneck(
        self, metrics: Dict[str, Any], rtl_data: Dict[str, Any]
    ) -> str:
        """Find module contributing most to latency."""
        return self._find_latency_bottleneck_recursive(metrics, rtl_data)

    def _find_latency_bottleneck_recursive(
        self, metrics: Dict[str, Any], rtl_data: Dict[str, Any]
    ) -> str:
        ...
        return self._find_worst_latency_module_recursive(
            hierarchy_tree, modules_performance
        )

    def _find_worst_latency_module_recursive(
        self, hierarchy_node: Dict, modules_performance: Dict
    ) -> str:
        ...
        if module_name in modules_performance:
            lat_value = (
                modules_performance[module_name]
                .get("performance", {})
                .get("Worst-caseLatency", 0)
            )
            current_latency = self._safe_int(lat_value, 0)

        self._log(
            f"[ResourceConstrainedLatencyObjective] Checking module {module_name}: Latency = {current_latency}"
        )
        ...
        if not children:
            ...
            self._log(
                f"[ResourceConstrainedLatencyObjective] Leaf bottleneck found: {module_name} (Latency = {current_latency})"
            )
            return module_name
        ...
        if worst_child_node:
            ...
            return self._find_worst_latency_module_recursive(
                worst_child_node, modules_performance
            )
        else:
...
class ResourceConstrainedIILatencyObjective(ObjectiveFunction):
    ...
    def evaluate(
        ...
        if "metrics" in metrics and self.top_module in metrics["metrics"]:
            ...
            if latency_value is None or (
                isinstance(latency_value, str)
                and latency_value.lower() in ("undef", "-", "")
            ):
                latency = float("inf")
            else:
                try:
                    latency = (
                        int(latency_value)
                        if not isinstance(latency_value, float)
                        else latency_value
                    )
                except (ValueError, TypeError):
                    latency = float("inf")
            ...
            if resource_violations:
                ...
                details["bottleneck_module"] = self._find_performance_bottleneck(
                    metrics, rtl_data, ii, latency
                )
                return ObjectiveResult.NEEDS_IMPROVEMENT, details
        else:
            details["bottleneck_module"] = self.top_module
    ...
    def _find_performance_bottleneck(
        self,
        metrics: Dict[str, Any],
        rtl_data: Dict[str, Any],
        ii: float,
        latency: float,
    ) -> str:
