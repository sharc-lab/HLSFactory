...
class InputCAnalyzer:
    ...
    def generate_input_c_hierarchy(self):
        ...
        if translation_unit.diagnostics:
            if self.logger:
                ...
                for diag in translation_unit.diagnostics:
                    self.logger.print(
                        f"  [{diag.severity}] {diag.spelling} (Line {diag.location.line}, Col {diag.location.column})"
                    )
            else:
                print("Diagnostics:")
                for diag in translation_unit.diagnostics:
        ...
        def print_ast(node, indent=0):
            if self.logger:
                self.logger.print(
                    "  " * indent + f"{node.kind} '{node.spelling}' <{node.location}>"
                )
            else:
                print(
                    "  " * indent + f"{node.kind} '{node.spelling}' <{node.location}>"
        ...
        def extract_children(node):
            ...
            def extract_function(stmt, append_list):
                ...
                if stmt.referenced and stmt.referenced.location.file:
                    ...
                    if self.logger:
                        self.logger.print(
                            "Remove call", call_file, src_file, stmt.spelling
                        )
                    else:
                    ...
                    append_list.append(
                        {
                            ...
                            "file": stmt.location.file.name
                            if stmt.location.file
                            else None,
                        }
                    )
                        ...
                        def collect_subloops(subnode, indent=2):
                            ...
                            for c in subnode.get_children():
                                if c.kind == CursorKind.LABEL_STMT:
                                    ...
                                    if (
                                        subtarget
                                        and subtarget.kind == CursorKind.FOR_STMT
                                    ):
                                        ...
                                        subloop = {
                                            ...
                                            "file": c.location.file.name
                                            if c.location.file
                                            else None,
                                            "children": collect_subloops(
                                                subtarget, indent + 1
                                            ),
                                        }
        ...
        for func in functions:
            for line_no in range(func["start_line"], func["end_line"] + 1):
                if line_no in pragma_lines:
                    ...
                    func["pragma_loc"].append(
                        (self.filename, line_no + 1)
    ...
    def pretty_print_input_c_hierarchy(self):
        def print_children(children, indent=2):
            for child in children:
                ...
                if child["type"] == "loop":
                    print(
                        f"{prefix}- Loop: {child['label']} (file {os.path.basename(child['file'])}:{child['line']})"
                    )
                    print_children(child["children"], indent + 2)
                elif child["type"] == "call":
                    print(
                        f"{prefix}- Call to: {child['name']} (file {os.path.basename(child['file'])}:{child['line']})"
                    )

        print("\n=== Function Hierarchy ===")
        for func in self.input_c_hierarchy:
            print(
                f"Function '{func['name']}' ({func['kind']}) [file {os.path.basename(func['file'])}:{func['start_line']}–{func['end_line']}]"
            )
            if func["pragma_loc"]:
                print("  Pragmas:")
    ...
    def save_pretty_print_hierarchy_to_txt(self, output_path=None):
        ...
        if self.input_c_hierarchy is None:
            raise ValueError(
                "Hierarchy not generated. Call generate_input_c_hierarchy() first."
            )
