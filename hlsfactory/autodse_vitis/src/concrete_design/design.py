...
class ConcreteDesign:
    def __init__(
        ...
        self.source_files = list(self.source_dir.glob("*.cpp")) + list(
            self.source_dir.glob("*.h")
        )
        ...
        if self.work_dir.exists():
            if force_overwrite:
                if self.logger:
                    self.logger.print(
                        f"[ConcreteDesign] Overwriting existing work_dir: {self.work_dir}"
                    )
                else:
                    print(
                        f"[ConcreteDesign] Overwriting existing work_dir: {self.work_dir}"
                ...
                if self.logger:
                    self.logger.print(
                        f"[ConcreteDesign] work_dir {self.work_dir} already exists. Use exstisting files or set force_overwrite=True to overwrite."
                    )
                else:
                    print(
                        f"[ConcreteDesign] work_dir {self.work_dir} already exists. Use exstisting files or set force_overwrite=True to overwrite."
            ...
            if self.logger:
                self.logger.print(
                    f"[ConcreteDesign] Creating new work_dir: {self.work_dir}"
                )
            else:
    ...
    def run_vitis_hls(self, timeout_min=10) -> str:
        ...
        try:
            if self.logger:
                self.logger.print(
                    f"[ConcreteDesign] Running Vitis HLS in {self.work_dir} using {tcl_name}..."
                )
            else:
                print(
                    f"[ConcreteDesign] Running Vitis HLS in {self.work_dir} using {tcl_name}..."
            ...
            if self.logger:
                self.logger.print(
                    f"[ConcreteDesign] ⏱ Timeout after {timeout_min} min in {self.work_dir}"
                )
            else:
                print(
                    f"[ConcreteDesign] ⏱ Timeout after {timeout_min} min in {self.work_dir}"
            ...
            if self.logger:
                self.logger.print(
                    f"[ConcreteDesign] ❌ Vitis HLS failed in {self.work_dir}: {e}"
                )
            else:
    ...
    def analyze_source(self):
        if self.logger:
            self.logger.print(
                f"[ConcreteDesign] Analyzing source code in {self.work_dir}..."
            )
        else:
        ...
        if not tcl_info["cpp_with_top_function"]:
            if self.logger:
                self.logger.print(
                    f"[ConcreteDesign] ❌ Could not find .cpp file containing top function '{tcl_info['top_function']}'"
                )
            else:
                print(
                    f"[ConcreteDesign] ❌ Could not find .cpp file containing top function '{tcl_info['top_function']}'"
        ...
        if self.logger:
            self.logger.print(
                f"[ConcreteDesign] Found top function '{tcl_info['top_function']}' in {tcl_info['cpp_with_top_function']}"
            )
            self.logger.print(
                f"[ConcreteDesign] Source files: {[str(f) for f in tcl_info['source_files']]}"
            )
        else:
            print(
                f"[ConcreteDesign] Found top function '{tcl_info['top_function']}' in {tcl_info['cpp_with_top_function']}"
    ...
    def analyze_csynth(self):
        ...
        _metrics = csynth_analyzer.parse_csynth_report()
    ...
    def find_source_files_from_tcl(self):
        ...
        source_files_list_match = re.search(
            r"set\s+source_files\s+\[list\s+([^\]]+)\]", tcl_content, re.DOTALL
        )
            ...
            for cpp_file in resolved_files:
                if cpp_file.suffix == ".cpp":
                    try:
                        ...
                        if self.logger:
                            self.logger.print(
                                f"Warning: Could not read {cpp_file}: {e}"
                            )
                        else:
