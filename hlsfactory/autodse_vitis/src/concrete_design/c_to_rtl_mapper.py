...
class CToRTLMapper:
    ...
    def build_c_hierarchy_index(self):
        ...
        if self.logger:
            self.logger.print(
                f"✓ Indexed {len(self.c_functions_by_name)} functions and {len(self.c_loops_by_line)} loops"
            )
        else:
            print(
                f"✓ Indexed {len(self.c_functions_by_name)} functions and {len(self.c_loops_by_line)} loops"
                ...
                if "_" in loop_pattern:
                    ...
                    if len(loop_parts) >= 2:
                        ...
                        if flattened_parts:
                            ...
                            for (
                                part
                            ) in loop_parts:  # Use all parts, not just flattened ones
                                for line, loop_info in self.c_loops_by_line.items():
                            ...
                            if original_loops:
                                self.final_mappings[module_name] = {
                                    "type": "flattened",
                                    "c_loops": [
                                        loop["label"] for loop in original_loops
                                    ],
                                    "c_lines": [
                                        loop["line"] for loop in original_loops
                                    ],
                                    # "start_lines": [loop["start_line"] for loop in original_loops],
                                    # "end_lines": [loop["end_line"] for loop in original_loops],
                                    "start_lines": min(
                                        loop["start_line"] for loop in original_loops
                                    ),
                                    "end_lines": max(
                                        loop["end_line"] for loop in original_loops
                                    ),
    ...
    def map_unmapped_to_parent(self):
        ...
        def find_immediate_parent_in_hierarchy(target_module, node, path=[]):
            ...
            for callee in node.get("callees", []):
                ...
                result = find_immediate_parent_in_hierarchy(
                    target_module, callee, current_path
                )
                if result:
                    return result
        ...
        def find_any_parent_in_hierarchy(target_module, node, path=[]):
            ...
            if (
                node_name in llvm_to_csynth_map
                and llvm_to_csynth_map[node_name] == target_module
            ):
            ...
            for callee in node.get("callees", []):
                result = find_any_parent_in_hierarchy(
                    target_module, callee, current_path
                )
                if result:
                    return result
        ...
        def _create_parent_mapping(node_name, path):
            ...
            if node_legalized in self.final_mappings:
                ...
                # Handle both regular and flattened mappings
                # Flattened mappings use start_lines/end_lines (singular values)
                # Regular mappings use start_line/end_line
                return {
                    "type": "function",
                    "c_function": existing_mapping.get("c_function"),
                    "c_line": existing_mapping.get(
                        "c_line", existing_mapping.get("start_lines")
                    ),
                    "start_line": existing_mapping.get(
                        "start_line", existing_mapping.get("start_lines")
                    ),
                    "end_line": existing_mapping.get(
                        "end_line", existing_mapping.get("end_lines")
                    ),
                    "c_file": existing_mapping.get("c_file"),
        ...
        for module_name in unmapped_modules:
            ...
            parent_mapping = find_immediate_parent_in_hierarchy(
                module_name, self.llvm_ir_hierarchy
            )

            # If no immediate parent found, try any parent
            if not parent_mapping:
                parent_mapping = find_any_parent_in_hierarchy(
                    module_name, self.llvm_ir_hierarchy
                )
            ...
            if parent_mapping:
                ...
                if self.logger:
                    self.logger.print(
                        f"  ✓ {module_name} -> {parent_mapping.get('c_function')}"
                    )
                else:
    ...
    def apply_legalization(self):
        ...
        if "legalizing" in self.transform_messages:
            ...
            if (
                hasattr(self, "csynth_module_info")
                and "metrics" in self.csynth_module_info
            ):
                for csynth_module in self.csynth_module_info["metrics"]:
                    if (
                        csynth_module not in new_mappings
                        and csynth_module in reverse_name_map
                    ):
                        ...
                        if original_name in self.llvm_modules:
                            ...
                            if "line" in llvm_info and isinstance(
                                llvm_info["line"], int
                            ):
                                line = llvm_info["line"]
                                ...
                                if line in self.c_loops_by_line:
                                    ...
                                    if base_func_name in self.c_functions_by_name:
                                        func_info = self.c_functions_by_name[
                                            base_func_name
                                        ]
                                        new_mappings[csynth_module] = {
                                            ...
                                            "start_line": func_info[
                                                "start_line"
                                            ],  # function definition boundaries
                                            "end_line": func_info[
                                                "end_line"
                                            ],  # function definition boundaries
                                            "function_def_line": func_info[
                                                "start_line"
                                            ],  # function definition line
                                            "function_def_start_line": func_info[
                                                "start_line"
                                            ],
                                            "function_def_end_line": func_info[
                                                "end_line"
                                            ],
                                            "c_file": func_info[
                                                "file"
    ...
    def validate_against_csynth(self):
        ...
        if self.logger:
            ...
            self.logger.print(
                f"  Successfully mapped: {len(mapped_modules & csynth_modules)}"
            )
            self.logger.print(f"  Unmapped csynth modules: {len(unmapped_csynth)}")
            self.logger.print(
                f"  Mapped but missing from csynth: {len(missing_from_csynth)}"
            )
        else:
            print("\n📊 Validation Results:")
        ...
        for mapping_type, mappings in summary.items():
            if self.logger:
                ...
                if mappings:
                    ...
                    if len(mappings) > 1:
                        other_names = [
                            name for name, _ in mappings[1:6]
                        ]  # Show up to 5 more
                        self.logger.print(
                            f"    Other examples: {', '.join(other_names)}"
                        )
                        if len(mappings) > 6:
    ...
    def annotate_csynth_with_c_info(self, output_file: str = None):
        ...
        for module_name, metrics in self.csynth_module_info.get("metrics", {}).items():
            module_info = {"performance": metrics, "c_source": None}
        ...
        if self.logger:
            ...
            self.logger.print(f"  Total csynth modules: {len(csynth_modules)}")
            self.logger.print(
                f"  Successfully annotated: {len(mapped_modules & csynth_modules)}"
            )
    ...
    def run_analysis(self):
        ...
        _summary = self.generate_mapping_summary()
        self.show_examples()
