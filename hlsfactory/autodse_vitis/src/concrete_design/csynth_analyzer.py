...
class CSynthAnalyzer:
    ...
    def __init__(self, work_dir):
        ...
        self.xml_path = os.path.join(
            self.csynth_report_dir, "syn", "report", "csynth.xml"
        )
    ...
    def find_csynth_report_dir(self, work_dir):
        ...
        for proj in os.scandir(work_dir):
            ...
            for sub in os.scandir(proj.path):
                ...
                if self.logger:
                    self.logger.print(
                        f"Checking subfolder: {sub.path} with entries: {entries}"
                    )
                else:
    ...
    def parse_hierarchy_instance(self, instance_elem):
        ...
        node["InstName"] = (
            inst_name_elem.text.strip() if inst_name_elem is not None else "UnknownInst"
        )
        node["ModuleName"] = (
            module_name_elem.text.strip()
    ...
    def parse_csynth_report(self):
        ...
        if not os.path.isfile(self.xml_path):
            error_lines = self.parse_error_log(
                os.path.join(self.csynth_report_dir, "solution.log")
            )
            self.csynth_module_info = {
        ...
        for module in mod_info_elem.findall("Module"):
            ...
            if perf_elem is not None:
                ...
                if overall_lat is not None:
                    ...
                    metrics["Best-caseLatency"] = (
                        best_lat.text.strip() if best_lat is not None else None
                    )
                    metrics["Average-caseLatency"] = (
                        avg_lat.text.strip() if avg_lat is not None else None
                    )
                    metrics["Worst-caseLatency"] = (
                        worst_lat.text.strip() if worst_lat is not None else None
                    )
                    ...
                    if ii is not None and ii.text:
                        ii_text = ii.text.strip()
                        # Handle 'undef' or other non-numeric values
                        if ii_text.lower() == "undef" or ii_text == "-":
                            metrics["PipelineInitiationInterval"] = None
                        else:
                            try:
                                metrics["PipelineInitiationInterval"] = max(
                                    map(int, ii_text.split("~"))
                                )
                ...
                if loop_lat_elem is not None:
                    ...
                    for loop_elem in loop_lat_elem:
                        ...
                        loop_display_name = (
                            loop_name_elem.text.strip()
                            if loop_name_elem is not None
                            else loop_id
                        )
                        ...
                        loop_info.update(
                            {
                                "Slack": slack.text.strip()
                                if slack is not None
                                else None,
                                "TripCount": trip_count.text.strip()
                                if trip_count is not None
                                else None,
                                "Loop-Best-caseLatency": latency.text.strip()
                                if latency is not None
                                else None,
                                "AbsoluteTimeLatency": abs_time_lat.text.strip()
                                if abs_time_lat is not None
                                else None,
                                "IterationLatency": iter_lat.text.strip()
                                if iter_lat is not None
                                else None,
                                "PipelineII": pipeline_ii.text.strip()
                                if pipeline_ii is not None
                                else None,
                                "PipelineDepth": pipeline_depth.text.strip()
                                if pipeline_depth is not None
                                else None,
                                "PipelineType": pipeline_type.text.strip()
            ...
            if area_elem is not None:
                ...
                if res_elem is not None:
                    ...
                    metrics["BRAM_18K"] = (
                        int(bram.text.strip())
                        if bram is not None and bram.text.strip().isdigit()
                        else 0
                    )
                    metrics["DSP"] = (
                        int(dsp.text.strip())
                        if dsp is not None and dsp.text.strip().isdigit()
                        else 0
                    )
                    metrics["FF"] = (
                        int(ff.text.strip())
                        if ff is not None and ff.text.strip().isdigit()
                        else 0
                    )
                    metrics["LUT"] = (
                        int(lut.text.strip())
                        if lut is not None and lut.text.strip().isdigit()
                        else 0
                    )
                    metrics["URAM"] = (
                        int(uram.text.strip())
                        if uram is not None and uram.text.strip().isdigit()
                        else 0
                    )
    ...
    def parse_transform_messages(self):
        """
        Parses autopilot.flow.log for HLS transform events:
        ...
        Raises FileNotFoundError if autopilot.flow.log is missing.
        Returns a dict with lists under keys 'unrolling', 'inlining', 'flattening'.
        """
        # Try new log format first (Vitis 2025.1+), fall back to old XML format
        log_file = os.path.join(
            self.csynth_report_dir, ".autopilot", "db", "autopilot.flow.log"
        )

        if os.path.isfile(log_file):
            return self._parse_flow_log(log_file)
        else:
            # Fall back to old XML format
            return self._parse_message_xml()

    def _parse_flow_log(self, log_file):
        """
        Parses autopilot.flow.log (new Vitis 2025.1+ format) for transform events.
        Format examples:
          - INFO: [HLS 214-186] Unrolling loop 'L6' (3mm.cpp:34:5) in function 'kernel_3mm' completely with a factor of 60 (3mm.cpp:4:0)
          - INFO: [HLS 214-188] Unrolling loop 'L7' (3mm.cpp:58:5) in function 'kernel_3mm' partially with a factor of 8 (3mm.cpp:4:0)
          - INFO: [XFORM 203-541] Flattening a loop nest 'L0' (3mm.cpp:21:5) in function 'kernel_3mm'.
        """
        unrolling = []
        inlining = []
        flattening = []
        ...
        legal_re = re.compile(r"Legalizing function name '([^']+)' to '([^']+)'")

        with open(log_file, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                line = line.strip()

                # Check for unrolling messages (214-186 = complete, 214-188 = partial)
                if "HLS 214-186" in line or "HLS 214-188" in line:
                    is_complete = "HLS 214-186" in line
                    m = (
                        ur_complete.search(line)
                        if is_complete
                        else ur_partial.search(line)
                    )
                    if m:
                        ...
                        unrolling.append(
                            {
                                "loop": m.group(1),
                                "loop_loc": {
                                    "file": os.path.basename(loop_file),
                                    "line": int(loop_line),
                                    "column": int(loop_col),
                                },
                                "func": m.group(3),
                                ...
                                "func_loc": {
                                    "file": os.path.basename(func_file),
                                    "line": int(func_line),
                                    "column": int(func_col),
                                },
                            }
                        ...
                        unparsed.append({"line": line})

                # Check for inlining messages (214-178 or XFORM 203-602)
                elif "HLS 214-178" in line or "XFORM 203-602" in line:
                    mx = inline_xform.search(line)
                    if mx:
                        inlining.append(
                            {
                                "callee": mx.group(1),
                ...
                # Check for flattening messages (XFORM 203-541)
                elif "XFORM 203-541" in line:
                    mf = flatten_re.search(line)
                    if mf:
                ...
                # Check for legalizing messages (SYN 201-103)
                elif "SYN 201-103" in line or "HLS 201-103" in line:
                    ml = legal_re.search(line)
                    if ml:
                        legalizing.append(
                            {"original": ml.group(1), "legalized": ml.group(2)}
                        )
                    else:
                        legalizing.append({"message": line})
    ...
    def _parse_message_xml(self):
        """
        Parses message_syn.xml (old Vitis 2024.2 and earlier format) for transform events.
        This is a fallback for older Vitis versions.
        """
        msg_file = os.path.join(
            self.csynth_report_dir, ".autopilot", "db", ".message_syn.xml"
        )
        if not os.path.isfile(msg_file):
            raise FileNotFoundError(
                f"Neither autopilot.flow.log nor message_syn.xml found in '{self.csynth_report_dir}'"
            )
    ...
    def save_transform_messages_to_json(self, output_path=None):
        ...
        if not hasattr(self, "transform_messages") or not self.transform_messages:
            raise ValueError(
                "Transform messages not parsed. Call parse_transform_messages() first."
            )
    ...
    def parse_llvm_ir_hierarchy(self):
        ...
        self.llvm_ir_hierarchy = LLVMIRParser(
            out_ll, Path(self.log_dir, "input_c_hierarchy.json")
        )  # path to .ll file
        ...
        if hasattr(self, "csynth_module_info") and self.csynth_module_info:
            if (
                "top_module" in self.csynth_module_info
                and "ModuleName" in self.csynth_module_info["top_module"]
            ):
    ...
    def save_llvm_ir_hierarchy_to_json(self, output_path=None):
        self.llvm_ir_hierarchy.save_hierarchy_to_json(
            Path(self.log_dir, "llvm_ir_hierarchy.json")
        )

    def save_pretty_print_llvm_ir_hierarchy_to_txt(self, output_path=None):
        self.llvm_ir_hierarchy.save_pretty_print_hierarchy_to_txt(
            Path(self.log_dir, "llvm_ir_hierarchy.txt")
        )
    ...
    def analyze_C_to_RTL_module_mapping(self, input_c_hierarchy_json):
        ...
        if not os.path.isfile(input_c_hierarchy_json):
            raise FileNotFoundError(
                f"Hierarchy JSON not found: {input_c_hierarchy_json}"
            )
        """
        Parses message_syn.xml to extract C-to-RTL mapping events and links
        them back to the original C hierarchy nodes.
