...
In contrast, "concrete" designs are those that can be directly synthesized and built by HLS or implementation tools via a "tool" flow. These designs can be provided as is by the user, for instance, a completed hardware kernel ready to be processed by Vitis HLS or a C++ kernel with a configured `synth.tcl` ready for Siemens Catapult.
